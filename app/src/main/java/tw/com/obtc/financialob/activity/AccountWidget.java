...
import static tw.com.obtc.financialob.utils.EnumUtils.selectEnum;

public class AccountWidget extends AppWidgetProvider {

    private static final Uri CONTENT_URI = Uri.parse("content://tw.com.obtc.financialob/accountwidget");

    private static final String WIDGET_UPDATE_ACTION = "tw.com.obtc.financialob.UPDATE_WIDGET";
    private static final String PREFS_NAME = "tw.com.obtc.financialob.activity.AccountWidget";
    ...
    public void onReceive(Context context, Intent intent) {
        Log.d("financialob", "onReceive intent " + intent);
    ...
    private static void updateWidgets(Context context, AppWidgetManager manager, int[] appWidgetIds, boolean nextAccount) {
        Log.d("financialob", "updateWidgets " + Arrays.toString(appWidgetIds) + " -> " + nextAccount);
        for (int id : appWidgetIds) {
            ...
            if (appWidgetInfo != null) {
                ...
                if (MyPreferences.isWidgetEnabled(context)) {
                    ...
                    Log.d("financialob", "using provider " + providerClass);
    ...
    private static RemoteViews buildUpdateForCurrentAccount(Context context, int widgetId, int layoutId, Class providerClass, long accountId) {
        ...
        try {
            ...
            if (a != null) {
                Log.d("financialob", "buildUpdateForCurrentAccount building update for " + widgetId + " -> " + accountId);
                return updateWidgetFromAccount(context, widgetId, layoutId, providerClass, a);
            } else {
                Log.d("financialob", "buildUpdateForCurrentAccount not found " + widgetId + " -> " + accountId);
    ...
    private static RemoteViews buildUpdateForNextAccount(Context context, int widgetId, int layoutId, Class providerClass, long accountId) {
        ...
        try {
            try (Cursor c = db.getAllActiveAccounts()) {
                ...
                if (count > 0) {
                    Log.d("financialob", "buildUpdateForNextAccount " + widgetId + " -> " + accountId);
                    if (count == 1 || accountId == -1) {
                        ...
                        while (c.moveToNext()) {
                            ...
                            if (a.id == accountId) {
                                found = true;
                                Log.d("financialob", "buildUpdateForNextAccount found -> " + accountId);
                            } else {
                                if (found) {
                                    Log.d("financialob", "buildUpdateForNextAccount building update for -> " + a.id);
                                    return updateWidgetFromAccount(context, widgetId, layoutId, providerClass, a);
                                }
                            }
                        }
                        c.moveToFirst();
                        Account a = EntityManager.loadFromCursor(c, Account.class);
                        Log.d("financialob", "buildUpdateForNextAccount not found, taking the first one -> " + a.id);
                        return updateWidgetFromAccount(context, widgetId, layoutId, providerClass, a);
                    }
                }
